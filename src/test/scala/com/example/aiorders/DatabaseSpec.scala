...
import com.example.aiorders.store.{PostgresUserStore, UserStore}
...
abstract class DatabaseSpec extends CatsEffectSuite with TestContainerForAll {
  ...
  def cleanDatabase(implicit transactor: Transactor[IO]): IO[Unit] =
    sql"TRUNCATE TABLE users RESTART IDENTITY CASCADE".update.run.transact(transactor).void
